...
ssize_t mywrite(int fd, const void *buf, size_t count)
{
  ...
  ssize_t resposta_write = 0;
  __asm__("mov %0, %%edi" : : "r"(fd));
  __asm__("mov %0, %%rsi" : : "r"(buf));
  __asm__("mov %0, %%rdx" : : "r"(count));
  __asm__("mov $1, %rax");
  __asm__("syscall");
  __asm__("mov %%rax, %0": "=r"(resposta_write):);
  return resposta_write;
}

int myclose(int fd)
{
  // return close(fd);
  int resposta_close = 0;
  __asm__("mov %0, %%edi" : : "r"(fd));
  __asm__("mov $3, %rax");
  __asm__("syscall");
  __asm__("mov %%eax, %0": "=r"(resposta_close):);
  return resposta_close;  
}
